...
// </NP1 Specific>
    ...
    */
	 ...
	 * @param[in] len: Amount of samples in 'data'.
	 ...
	 *
	 * This prepares the output SMA channel to playback the waveform programmed with '\ref waveplayer_writeBuffer'.
	 * The Waveplayer must be triggered by a signal in the switch matrix. (See \ref switchmatrix_set)
	 * By default, \ref SM_Input_SWTrigger2 is bound as the WavePlayer software trigger.
	 *
     * @param[in] slotID: Id of the slot in the PXIe chassis or the virtual slot for OneBox.
	 * @param[in] single_shot: If true, the Waveplayer will play the programmed waveform once after trigger. if false, the waveform is repeat until rearmed in other mode.
	 * @returns SUCCESS if successful. NOTSUPPORTED if this functionality is not supported by the device.
	 */
    ...
    *          WRONG_SLOT in case a slot number outside the valid range is entered,\n
    *          WRONG_PORT in case a port number outside the valid range is entered,\n
    *          ERROR_SR_CHAIN_1 in case the SR_OUT_OK bit is not okay when writing SR_CHAIN_1,\n
    *          ERROR_SR_CHAIN_2 in case the SR_OUT_OK bit is not okay when writing SR_CHAIN_2,\n
    *          ERROR_SR_CHAIN_3 in case the SR_OUT_OK bit is not okay when writing SR_CHAIN_3.\n
    */
...
#define NP_APIC __stdcall
	extern "C" {
		...
		NP_EXPORT NP_ErrorCode NP_APIC np_writeProbeConfiguration(int slotID, int portID, int dockID, bool reac_check);
